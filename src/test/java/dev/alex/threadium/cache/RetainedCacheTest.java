...
class RetainedCacheTest {
    @Test
    void hitMissAndCollisionSafeKeys() {
        RetainedCache<Key, Value> cache = new RetainedCache<>(4, 100);
        Key first = new Key("first", 7);
        Key collision = new Key("second", 7);
        assertEquals(
                RetainedCache.Outcome.MISS,
                cache.getOrBuild(first, 1, 1, () -> new Value(10)).outcome());
        assertEquals(
                RetainedCache.Outcome.HIT,
                cache.getOrBuild(first, 2, 1, () -> fail("must not rebuild")).outcome());
        assertEquals(
                RetainedCache.Outcome.MISS,
                cache.getOrBuild(collision, 3, 1, () -> new Value(10)).outcome());
        assertEquals(2, cache.size());
    }

    @Test
    void evictsLeastRecentlyUsedAndAccountsBytes() {
    ...
    @Test
    void negativeEntryBacksOffUntilGenerationChanges() {
        RetainedCache<String, Value> cache = new RetainedCache<>(4, 100);
        AtomicInteger attempts = new AtomicInteger();
        assertEquals(
                RetainedCache.Outcome.NEGATIVE,
                cache.getOrBuild("x", 1, 1, () -> {
    ...
    @Test
    void idleSessionAndRuntimeDisableCleanupReleaseResources() {
        RetainedCache<String, Value> cache = new RetainedCache<>(4, 100);
