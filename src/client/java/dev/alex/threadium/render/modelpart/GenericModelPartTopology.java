...
public record GenericModelPartTopology(List<Node> nodes, StructuralKey key) {
    ...
    private static void visit(ModelPart part, int parent, String path, int depth, int maxBones, int maxDepth,
                              List<Node> out, List<Long> exact) {
        if (depth > maxDepth || out.size() >= maxBones) throw new IllegalArgumentException("ModelPart hierarchy limit exceeded");
        int index = out.size();
        out.add(new Node(part, parent, index, path));
        ModelPartAccessor access = (ModelPartAccessor) (Object) part;
        exact.add((long) parent); exact.add((long) access.threadium$cubes().size()); exact.add((long) access.threadium$children().size());
        ...
        for (ModelPart.Cube cube : access.threadium$cubes()) {
            ...
            for (ModelPart.Polygon polygon : cube.polygons) {
                ...
                for (ModelPart.Vertex v : polygon.vertices()) {
                    exact.add((long) Float.floatToRawIntBits(v.worldX())); exact.add((long) Float.floatToRawIntBits(v.worldY()));
                    exact.add((long) Float.floatToRawIntBits(v.worldZ())); exact.add((long) Float.floatToRawIntBits(v.u())); exact.add((long) Float.floatToRawIntBits(v.v()));
                }
            }
        }
