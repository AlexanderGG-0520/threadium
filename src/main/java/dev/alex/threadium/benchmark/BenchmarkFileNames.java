...
public final class BenchmarkFileNames {
    ...
    public static String base(Instant timestamp, ModelPartBenchmarkMode mode, String scene, int trial) {
        return TIME.format(timestamp) + '_' + mode.name().toLowerCase(java.util.Locale.ROOT) + '_' + sanitize(scene)
    ...
    static String sanitize(String value) {
        String clean = value.toLowerCase(java.util.Locale.ROOT)
                .replaceAll("[^a-z0-9_-]+", "-")
                .replaceAll("^-+|-+$", "");
        return clean.isEmpty() ? "scene" : clean;
    }
}
