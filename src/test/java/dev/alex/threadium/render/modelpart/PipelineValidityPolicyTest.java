...
class PipelineValidityPolicyTest {
    ...
    @Test
    void onlyValidCurrentGenerationCanProceed() {
        ...
        assertFalse(new PipelineValidity(PipelineValidityState.VALID, GENERATION - 1, "old")
                .permitsReplacement(GENERATION));
    }

    @Test
    void compilationExceptionIsInvalidAndFallsBack() {
        PipelineValidity result = PipelineCompilationPolicy.evaluate(GENERATION, () -> {
        ...
        assertEquals(
                ...
                Blaze3dSubmissionPolicy.replacementResult(result, GENERATION, true));
        assertTrue(result.reason().contains("compile failed"));
    }

    @Test
    void validityIsIndependentPerSourcePipeline() {
        ...
        assertEquals(
                PipelineValidityState.UNINITIALIZED,
                table.get(cutout, GENERATION).state());
        assertEquals(
                PipelineValidityState.UNINITIALIZED,
                table.get(cutoutCull, GENERATION).state());
    }

    @Test
    void generationChangeMarksEveryPipelineStale() {
        ...
        assertEquals(
                PipelineValidityState.STALE, table.get(solid, GENERATION + 1).state());
        assertFalse(table.get(solid, GENERATION + 1).permitsReplacement(GENERATION + 1));
        assertEquals(
                PipelineValidityState.STALE, table.get(cutout, GENERATION + 1).state());
    }

    @Test
    void queueInsertionFailureStillPassesThrough() {
        PipelineValidity valid = new PipelineValidity(PipelineValidityState.VALID, GENERATION, "valid");
        assertEquals(
                ModelPartInterceptionResult.PASS_THROUGH,
                Blaze3dSubmissionPolicy.replacementResult(valid, GENERATION, false));
    }

    @Test
    void suppressionResultExistsOnlyAfterValidityAndInsertion() {
        PipelineValidity valid = new PipelineValidity(PipelineValidityState.VALID, GENERATION, "valid");
        assertEquals(
                ModelPartInterceptionResult.GPU_REPLACED,
                Blaze3dSubmissionPolicy.replacementResult(valid, GENERATION, true));
        for (PipelineValidityState state : List.of(
                PipelineValidityState.UNINITIALIZED,
                PipelineValidityState.INVALID,
                PipelineValidityState.STALE,
                PipelineValidityState.UNSUPPORTED_BACKEND)) {
            PipelineValidity rejected = new PipelineValidity(state, GENERATION, state.name());
            assertEquals(
                    ModelPartInterceptionResult.PASS_THROUGH,
                    Blaze3dSubmissionPolicy.replacementResult(rejected, GENERATION, true));
        }
    }

    @Test
    void invalidFallbackDoesNotImplySubmissionMetrics() {
