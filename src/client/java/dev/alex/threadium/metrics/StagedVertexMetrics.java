...
public final class StagedVertexMetrics {
    ...
    private static final DistributionBuckets sortedQuadBuckets = new DistributionBuckets(0, 1, 7, 31, 127, 511, 2047, 8191);
    ...
    public static void configure(boolean value) { enabled = value; }
    ...
    public static void recordGroup(long start) { if (enabled) { record(groupPreparation, start); STATE.get().frameGroups++; } }
    public static void recordUpload(long start) { record(uploadTotal, start); }
    public static void recordDecode(long start) { record(decodeSortingPoints, start); }
    public static void recordIndexGeneration(long start) { record(quadOrderingIndexGeneration, start); }
    public static void recordCpuCopy(long start) { record(cpuBufferCopy, start); }
    public static void recordGpuCopy(long start) { record(gpuUploadCall, start); }
    ...
    public static void cpuDataReady(StagedVertexBuffer buffer) {
        ...
        for (int index = 0, size = liveDraws.size(); index < size; index++) {
            ...
            long drawQuads = StagedMetricMath.quadCount(accessor.threadium$getPrimitiveTopology() == PrimitiveTopology.QUADS, vertexCount);
            ...
            add(draws, 1L); add(vertices, vertexCount); add(quads, drawQuads); add(stagedBytes, bytes);
            if (sorted) {
                ...
                add(sortedDraws, 1L); add(sortedQuads, drawQuads); add(generatedIndices, indices); add(stagedBytes, indexBytes);
        ...
        maximum(maxVerticesPerFrame, frameVertices); maximum(maxQuadsPerFrame, frameQuads);
        maximum(maxSortedQuadsPerFrame, frameSortedQuads); maximum(maxStagedBytesPerFrame, frameBytes);
    }
    ...
    public static String snapshotAndReset() {
        ...
        return "stagedVertex={frames=" + reset(frames) + ",groups=" + reset(groups) + ",draws=" + reset(draws)
                ...
                + ",maxQuadsPerFrame=" + reset(maxQuadsPerFrame) + ",maxSortedQuadsPerFrame=" + reset(maxSortedQuadsPerFrame)
                ...
                + ",decodeSortingPoints=" + decodeSortingPoints.snapshotAndReset().describe()
                + ",quadOrderingIndexGeneration=" + quadOrderingIndexGeneration.snapshotAndReset().describe()
    ...
    public record BenchmarkTiming(TimingAccumulator.Sample featurePreparation,TimingAccumulator.Sample groupPreparation) { }

    private static void record(TimingAccumulator metric, long start) { if (enabled && start != 0L) metric.record(System.nanoTime() - start); }
    private static void add(AtomicLong target, long value) { target.getAndUpdate(current -> DistributionBuckets.saturatingAdd(current, value)); }
    private static long add(long current, long value) { return DistributionBuckets.saturatingAdd(current, value); }
    private static void maximum(AtomicLong target, long value) { target.accumulateAndGet(value, Math::max); }
    private static long reset(AtomicLong value) { return value.getAndSet(0L); }

    private static final class State { long frameGroups; long cpuReadyNanos; }
}
