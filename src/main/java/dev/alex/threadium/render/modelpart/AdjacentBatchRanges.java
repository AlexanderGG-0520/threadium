...
public final class AdjacentBatchRanges {
    ...
    public static <T> List<Range> build(List<T> keys, boolean consolidate) {
        ...
        for(int first=0;first<keys.size();){int end=first+1;if(consolidate)while(end<keys.size()&&java.util.Objects.equals(keys.get(first),keys.get(end)))end++;result.add(new Range(first,end-first));first=end;}
