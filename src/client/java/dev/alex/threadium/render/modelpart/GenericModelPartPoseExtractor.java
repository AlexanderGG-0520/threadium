...
public final class GenericModelPartPoseExtractor {
    public ModelPartBoneData extract(GenericModelPartTopology topology) {
        ...
        for (GenericModelPartTopology.Node node : topology.nodes()) {
            ...
            var p = node.part();
            Matrix4f matrix = dev.alex.threadium.render.modelpart.TransformMath.compose(parent, p.x,p.y,p.z,p.xRot,p.yRot,p.zRot,p.xScale,p.yScale,p.zScale);
            ...
            out[base+16]=normal.m00(); out[base+17]=normal.m01(); out[base+18]=normal.m02(); out[base+19]=0;
            out[base+20]=normal.m10(); out[base+21]=normal.m11(); out[base+22]=normal.m12(); out[base+23]=0;
            out[base+24]=normal.m20(); out[base+25]=normal.m21(); out[base+26]=normal.m22(); out[base+27]=0;
        }
