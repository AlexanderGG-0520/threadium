...
public final class DifferentialExecutionScope {
    ...
    private static long observed, accepted, fallback, suppressions, totalAccepted, totalFallback;
    ...
    public static Token enter(Mode requested, String canonical) {
        ...
        observed = accepted = fallback = suppressions = totalAccepted = totalFallback = 0;
        fallbackReason = null;
    ...
    public static void completed(String canonical, ModelPartInterceptionResult result) {
        checkOwner();
        if (mode != Mode.CANDIDATE_REQUIRE_ACCEPT) return;
        if (result == ModelPartInterceptionResult.GPU_REPLACED) totalAccepted++;
        else totalFallback++;
        if (!matches(canonical)) return;
        observed++;
        if (result == ModelPartInterceptionResult.GPU_REPLACED) accepted++;
        else fallback++;
    }
