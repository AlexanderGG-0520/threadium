...
final class OpenGlRenderTarget implements AutoCloseable {
    ...
    Binding bind(RenderTarget target) {
        if (framebuffer == 0) framebuffer = GL30C.glGenFramebuffers();
        ...
        if (!(target.getColorTextureView() instanceof GlTextureView color))
            throw new IllegalStateException("non-OpenGL color target");
        ...
        if (target.useDepth && target.getDepthTextureView() instanceof GlTextureView depth)
            GL30C.glFramebufferTexture2D(
                    GL30C.GL_DRAW_FRAMEBUFFER,
        ...
        return new Binding(
                ...
                depthId,
                GL11C.GL_TEXTURE_2D,
                depthId == 0 ? 0 : ((GlTextureView) target.getDepthTextureView()).fboMipLevel(),
                1);
    }
