...
class SortedModelPartQuadsTest {
    ...
    void perspectiveOrderIsFarToNearAndStableForTies() {
        ...
        assertEquals(
                List.of(1, 2, 0, 3),
                refs.stream().map(SortedModelPartQuads.Reference::instanceIndex).toList());
    }

    @Test
    void keyUsesBoneThenRootTransform() {
        var boneMoved = SortedModelPartQuads.reference(0, 0, 0, 1, 0, 0, 0, bone(2, 0, 0), new Matrix4f());
        var rootMoved =
                SortedModelPartQuads.reference(0, 0, 0, 1, 0, 0, 0, bone(0, 0, 0), new Matrix4f().translate(2, 0, 0));
    ...
    void differentInstancesCanProduceDifferentQuadOrder() {
        ...
        refs.add(
                SortedModelPartQuads.reference(1, 0, 1, 1, 0, 0, 0, bone(0, 0, 0), new Matrix4f().translate(10, 0, 0)));
        SortedModelPartQuads.sort(refs);
        assertEquals(1, refs.getFirst().instanceIndex());
    }

    @Test
    void auditedSortOnUploadDescriptorsUseSortedPolicy() {
        var expected = java.util.Set.of(
                ModelPartPipelineDescriptor.ARMOR_TRANSLUCENT,
                ModelPartPipelineDescriptor.ENTITY_TRANSLUCENT,
                ModelPartPipelineDescriptor.ENTITY_TRANSLUCENT_CULL,
                ModelPartPipelineDescriptor.ENTITY_TRANSLUCENT_EMISSIVE,
                ModelPartPipelineDescriptor.BANNER_PATTERN,
                ModelPartPipelineDescriptor.BREEZE_WIND,
                ModelPartPipelineDescriptor.ENERGY_SWIRL,
                ModelPartPipelineDescriptor.EYES,
                ModelPartPipelineDescriptor.CRUMBLING);
        for (var descriptor : ModelPartPipelineDescriptor.values())
            assertEquals(
                    expected.contains(descriptor),
                    descriptor.submissionPolicy() == ModelPartPipelineDescriptor.SubmissionPolicy.SORTED_QUAD_STREAM,
                    descriptor.name());
    }
}
