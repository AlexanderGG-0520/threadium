...
enum ModelPartPipelineDescriptor {
    ...
    ENTITY_TRANSLUCENT_EMISSIVE(RenderPipelines.ENTITY_TRANSLUCENT_EMISSIVE, ShaderMode.EMISSIVE_OVERLAY, Ordering.ORDERED_ADJACENT),
    ...
    static final List<RenderPipeline> SOURCES = List.of(values()).stream().map(ModelPartPipelineDescriptor::source).toList();
    ...
    RenderPipeline source() { return source; }
    ShaderMode shaderMode() { return shaderMode; }
    Ordering ordering() { return ordering; }
    String canonicalName() { return name().toLowerCase(java.util.Locale.ROOT); }
    SubmissionPolicy submissionPolicy() { return switch(this) {
        case ARMOR_TRANSLUCENT,ENTITY_TRANSLUCENT,ENTITY_TRANSLUCENT_EMISSIVE,ENTITY_TRANSLUCENT_CULL,
             BANNER_PATTERN,BREEZE_WIND,ENERGY_SWIRL,EYES,CRUMBLING -> SubmissionPolicy.SORTED_QUAD_STREAM;
        case ENTITY_SOLID,ENTITY_CUTOUT_CULL,ENTITY_CUTOUT,ENTITY_CUTOUT_Z_OFFSET,WATER_MASK -> SubmissionPolicy.OPAQUE_BATCHED;
        default -> SubmissionPolicy.ORDERED_ADJACENT_BATCHED;
    }; }
    boolean perFaceLighting() { return switch(this) { case ARMOR_CUTOUT,ARMOR_DECAL,ARMOR_TRANSLUCENT,ENTITY_CUTOUT,ENTITY_CUTOUT_Z_OFFSET,ENTITY_CUTOUT_DISSOLVE,ENTITY_TRANSLUCENT,ENTITY_TRANSLUCENT_EMISSIVE -> true; default -> false; }; }
    boolean cardinalLighting() { return this==BANNER_PATTERN; }
    boolean alphaCutout() { return switch(this) { case ENTITY_SOLID,BANNER_PATTERN,EYES,OUTLINE_CULL,OUTLINE_NO_CULL,WATER_MASK -> false; default -> true; }; }
    boolean bindsOverlay() { return shaderMode.overlay || this==ARMOR_CUTOUT || this==ARMOR_DECAL || this==ARMOR_TRANSLUCENT; }
    boolean bindsLightmap() { return shaderMode.lightmap || this==ENERGY_SWIRL; }
    ...
    enum Ordering { ADJACENT, ORDERED_ADJACENT }
    enum SubmissionPolicy { OPAQUE_BATCHED, ORDERED_ADJACENT_BATCHED, SORTED_QUAD_STREAM, SINGLETON_ONLY }
    enum ShaderMode {
        ...
        ShaderMode(boolean overlay, boolean lightmap, boolean dissolveMask, boolean textureMatrix) {
            this.overlay = overlay; this.lightmap = lightmap; this.dissolveMask = dissolveMask; this.textureMatrix = textureMatrix;
        }
    }
}
