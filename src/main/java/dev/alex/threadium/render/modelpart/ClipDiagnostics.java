...
public final class ClipDiagnostics {
    ...
    public static Vector4f shaderTransform(Matrix4f projection, Matrix4f root,
                                           Matrix4f bone, Vector4f local) {
    ...
    public static boolean insideClipVolume(Vector4f clip, boolean zeroToOneDepth) {
        if (!Float.isFinite(clip.x) || !Float.isFinite(clip.y)
                || !Float.isFinite(clip.z) || !Float.isFinite(clip.w) || clip.w <= 0.0f) return false;
        float minimumDepth = zeroToOneDepth ? 0.0f : -clip.w;
        return clip.x >= -clip.w && clip.x <= clip.w
                && clip.y >= -clip.w && clip.y <= clip.w
                && clip.z >= minimumDepth && clip.z <= clip.w;
    }
}
