...
public final class RetainedCache<K, V extends RetainedCache.Resource> {
    ...
    public RetainedCache(int maxEntries, long maxBytes) {
        reconfigure(maxEntries, maxBytes);
    }
    ...
    public synchronized Lookup<V> getOrBuild(K key, long nowNanos, long generation, Supplier<V> builder) {
        ...
        return retained == null || retained.negative
                ? new Lookup<>(Outcome.NEGATIVE, null)
                : new Lookup<>(Outcome.MISS, retained.value);
    }
    ...
    private void release(Entry<V> entry) {
        if (!entry.negative) {
    ...
    private static final class Entry<V> {
        ...
        Entry(V value, long generation, long lastUsedNanos, boolean negative) {
            this.value = value;
