...
class DifferentialExecutionScopeTest {
    ...
    @Test
    void referenceBypassObservesWithoutAcceptanceOrSuppression() {
        try (var scope =
            ...
            assertEquals(0, result.suppressions());
        }
        assertFalse(DifferentialExecutionScope.active());
    }

    @Test
    void candidateRequiresExactDescriptor() {
        try (var scope = DifferentialExecutionScope.enter(
            ...
            DifferentialExecutionScope.completed("entity_solid", ModelPartInterceptionResult.GPU_REPLACED);
    ...
    @Test
    void candidateFallbackIsRetained() {
        try (var scope = DifferentialExecutionScope.enter(
                DifferentialExecutionScope.Mode.CANDIDATE_REQUIRE_ACCEPT, "entity_solid")) {
            DifferentialExecutionScope.completed("entity_solid", ModelPartInterceptionResult.PASS_THROUGH);
            assertEquals(1, scope.result().fallback());
        }
    }

    @Test
    void nestingIsRejectedAndFinallyRestoresNormal() {
        assertThrows(IllegalStateException.class, () -> {
            try (var ignored = DifferentialExecutionScope.enter(
                    DifferentialExecutionScope.Mode.REFERENCE_BYPASS, "entity_solid")) {
    ...
    @Test
    void resetClearsLeakedScope() {
        DifferentialExecutionScope.enter(DifferentialExecutionScope.Mode.REFERENCE_BYPASS, "entity_solid");
        DifferentialExecutionScope.reset();
        assertFalse(DifferentialExecutionScope.active());
        assertEquals(
                DifferentialExecutionScope.Mode.NORMAL,
                DifferentialExecutionScope.snapshot().mode());
    }
}
