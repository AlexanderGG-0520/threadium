...
public final class InstanceBatchSlicePlanner {
    ...
    public static List<Slice> plan(
            long uploadedBaseOffset, int stride, List<Integer> batchCounts, int uploadedInstanceCount) {
        if (uploadedBaseOffset < 0 || stride <= 0 || uploadedInstanceCount < 0)
            throw new IllegalArgumentException("invalid instance upload layout");
    ...
    public static Slice slice(long uploadedBaseOffset, int stride, int firstInstance, int instanceCount) {
        if (uploadedBaseOffset < 0 || stride <= 0 || firstInstance < 0 || instanceCount <= 0)
            throw new IllegalArgumentException("invalid instance batch layout");
    ...
    public static void requireCoverage(int plannedInstanceCount, int uploadedInstanceCount) {
        if (plannedInstanceCount != uploadedInstanceCount) {
            throw new IllegalStateException("ModelPart batch instance coverage mismatch: planned="
                    + plannedInstanceCount + ", uploaded=" + uploadedInstanceCount);
        }
    }
