...
public final class ThreadiumClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        ThreadiumConfig config = benchmarkMode.map(mode->ThreadiumConfig.load().forBenchmark(mode)).orElseGet(ThreadiumConfig::load);
        ThreadiumMetrics metrics = new ThreadiumMetrics(config);
        ThreadiumBenchmarkCommands.register();
        benchmarkMode.ifPresent(mode->{BenchmarkStartupOwnership ownership=BenchmarkStartupOwnership.forMode(mode);if(ownership.benchmarkController())ThreadiumBenchmark.initialize(mode,metrics);});
        ThreadiumLifecycle.initialize(config, metrics);
        if(benchmarkMode.isEmpty()||BenchmarkStartupOwnership.forMode(benchmarkMode.get()).modelPartRenderer())ModelPartRenderService.initialize(config);
