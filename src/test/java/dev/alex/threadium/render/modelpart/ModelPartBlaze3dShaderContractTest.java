...
class ModelPartBlaze3dShaderContractTest {
    @Test
    void normalOverlayCoordinatesRemainDecodedTexels() {
        assertArrayEquals(new int[] {0, 10}, decodePackedCoordinates(pack(0, 10)));
    }

    @Test
    void hurtOverlayRetainsVThree() {
        assertArrayEquals(new int[] {0, 3}, decodePackedCoordinates(pack(0, 3)));
    }

    @Test
    void overlayUsesDirectTexelsWhileLightStillDividesBySixteen() throws IOException {
    ...
    @Test
    void vertexPositionAppliesBoneRootThenViewThenProjectionExactlyOnce() throws IOException {
    ...
    @Test
    void instanceSlicePlanningContractRemainsIntact() {
        var slices =
                InstanceBatchSlicePlanner.plan(256, ModelPartLayouts.INSTANCE_STRIDE, java.util.List.of(1, 1, 1), 3);
        assertEquals(
                java.util.List.of(256L, 368L, 480L),
    ...
    private static int[] decodePackedCoordinates(int packed) {
        return new int[] {packed & 0xFFFF, packed >> 16 & 0xFFFF};
    }
