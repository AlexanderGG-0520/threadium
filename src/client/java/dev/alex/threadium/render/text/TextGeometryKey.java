...
public record TextGeometryKey(RetainedTextKey<Glyph> value) {
    ...
    public static TextGeometryKey capture(
        ...
        sequence.accept((index, style, codePoint) -> {
            glyphs.add(new Glyph(index, codePoint, style));
            return true;
        });
        return new TextGeometryKey(new RetainedTextKey<>(
                glyphs, x, y, color, shadow, bidirectional, backgroundColor, variant, generation));
    }
    ...
    public long resourceGeneration() {
        return value.resourceGeneration();
    }
